...
public:
    ...
    WebrtcApi& setBearerToken(std::string _token) { this->bearer_token = _token; }
    WebrtcApi& setBasicAuth(std::string _username, std::string _password) { this->basic_username = _username; this->basic_password = _password; }
    WebrtcApi& setApikey(std::string _apikey) { this->apikey = _apikey; }
    ...
    auto postWebRTCAuthenticate(
        ...
        if(_postWebRTCAuthenticateRequest.has_value()) {
            json["postWebRTCAuthenticateRequest"] = (_postWebRTCAuthenticateRequest.value());
        }

...
public:
    ...
    Oauth2Api& setBearerToken(std::string _token) { this->bearer_token = _token; }
    Oauth2Api& setBasicAuth(std::string _username, std::string _password) { this->basic_username = _username; this->basic_password = _password; }
    Oauth2Api& setApikey(std::string _apikey) { this->apikey = _apikey; }
    ...
    auto createClient(
        ...
        if(_postClientRequest.has_value()) {
            json["postClientRequest"] = (_postClientRequest.value());
        }
    ...
    auto editClient(
        ...
        if(_patchClientRequest.has_value()) {
            json["patchClientRequest"] = (_patchClientRequest.value());
        }
        auto req = drogon::HttpRequest::newHttpJsonRequest(json);
